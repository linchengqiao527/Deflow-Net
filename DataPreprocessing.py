...
def load_data(T=48, nb_flow=2, len_closeness=None, len_period=None, len_trend=None,
    ...
    timestamps=[]
    timestamps_all = list()
    for year in range(0, 180):
        for i in range(1,49):
            if i<10:
                temp = return_day(year)+"0"+str(i)
                temp.encode('utf-8')
                timestamps.append(temp)
            else:
                temp = return_day(year)  + str(i)
                temp.encode('utf-8')
                timestamps.append(temp)
    data = np.load("/home/xiaoxiaolin/PycharmProjects/DeepST-ResNet/data/New_TaxiSZ_50100_dealed.npy")#File name
    print(data.shape)
    data = data[:-1]
    data = data.reshape((data.shape[0]*data.shape[1],2,50,100))
    # print(timestamps)
